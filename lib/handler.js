...
async function getRecords(query, context) {
  const { month, category, type } = query;

  let { records } = await readData(context);
...
async function createRecord(body, context) {
...
async function batchDelete(body, context) {
  const { ids } = body || {};
...
// GET /api/stats 月度统计
async function getStats(query, context) {
  const { month } = query;
  const now = new Date();
  const targetMonth = month || `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;

  const { records } = await readData(context);
